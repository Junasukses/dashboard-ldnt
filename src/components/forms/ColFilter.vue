<template>
  <div class="relative colfilter flex items-center group">
    <select v-if="params.filterParams?.options" 
      :value="filterText" @focus="()=>isFocus=true" @blur="()=>isFocus=false"
      @input="(e) => (filterText = e.target.value)" @change="updateFilter"
    ...
    >
      ...
      <option class="cursor-pointer" v-for="dt in params.filterParams.options" :key="key+'-opt-'+dt" :value="dt">{{dt}} </option>
    </select>

    <date-picker v-else-if="params.filterParams?.range"
      :class="{
        'not-empty':filterText
      }"
      class="absolute py-1 rounded-sm hover:(bg-white)"
      :value="filterText?filterText.split('~'):null" @focus="()=>isFocus=true" @blur="()=>isFocus=false"
      @update:value="(val)=>{
        filterText = Array.isArray(val)?(val[0]? (val[0]+'~'+val[1]):null):val;
      }"
      type="date" value-type="format" format="DD/MM/YYYY"
      range
      @change="updateFilter()"
    ...
    >
      ...
      <template #input>
        <input
          class="px-1 cursor-pointer bg-gray-50 py-1.1 rounded-sm border w-full hover:(bg-white) focus:(shadow-md border-blue-400 bg-white placeholder-transparent) outline-none transition-all duration-200 placeholder-transparent"
          :class="{
            '!bg-white': filterText,
          ...
          @change="updateFilter"
          @keydown.esc="(e)=>{filterText=null;e.preventDefault();updateFilter()}"
        />
      </template>
    </date-picker>

    <input v-else
      class="px-1 bg-gray-50 py-1 rounded-sm border w-full hover:(bg-white) focus:(shadow-md border-blue-400 bg-white placeholder-transparent) outline-none transition-all duration-200 placeholder-transparent"
      :class="{
        '!bg-white': filterText,
        '!border-blue-400': filterText
      }"
    ...
    <icon fa="filter" :size="'sm absolute text-gray-400 transition-all duration-500 '+(isFocus||filterText?'right-1 !text-blue-400':'right-10/9 !left-1.5')" />
  </div>
</template>
