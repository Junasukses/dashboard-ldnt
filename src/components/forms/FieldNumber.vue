<template>
    <div
      ...
      <input :id="idInput" ref="input" :autofocus="autofocus ? 'autofocus' : false"
        :value="valueModel" :name="Math.random() - false" autocomplete="new-password" type="tel"
        :placeholder="placeholder" v-bind="bind"
        ...
        }" @keydown="numberKeydown" @keyup="numberKeyup" @input="onInput" @change="onInput" @focus="$emit('focus', true);isFocus = true;"
        @blur="isFocus = false; onBlurNumber();" />
      <icon v-if="faIcon" :fa="faIcon" :class="
        'absolute -left-1 fa-fw ' +
        (isFocus ? '!text-blue-500' : 'text-gray-600') + (isFocus ? ' animated animate-head-shake ' : '')
      " />
  
      <icon v-if="errorText" :title="errorText" fa="exclamation-circle"
        class="absolute left-0 fa-sm fa-fw text-red-400" />
      <icon v-else-if="check && !params" fa="check" class="absolute left-0 text-green-400" />
      <div v-show="isFocus" v-if="hints.length > 0"
        ...
        <ul class="bg-white border border-yellow-500 rounded p-1.5 shadow-sm">
          <li v-for="hint in hints" :key="'hint-' + hint" class="flex flex-row px-2 gap-3 place-items-center">
            <icon :class="errorText ? '!text-red-400' : '!text-yellow-500'" :fa="errorText ? 'exclamation' : 'info'"
              style="zoom: 90%" />
  ...
  <script>
  export default {
    emits: ['input', 'update:value', 'update:errorText', 'change', 'blur', 'keyup', 'keydown', 'focus'],
    props: {
      params: {
        type: Object,
        default() {
    ...
    methods: {
      ...
      getValue() {
        try {
          const val = parseFloat(
            this.valueModel.replace(/\./g, "").replace(/,/g, ".")
          );
      ...
      numberKeyup(event) {
        ...
        const formattedVal = parseFloat(
          ...
          minimumFractionDigits: value.split(",")[1]
        ...
        event.target.value = value.endsWith(",")
          ? formattedVal + ","
          : formattedVal;
        const realNumber = parseFloat(
          value.replace(/\./g, "").replace(/,/g, ".")
        );
        if (!isNaN(realNumber)) {
      ...
      onBlurNumber() {
        ...
        if (
          ...
          me.valueModel =
            formattedVal +
            (rawValue.includes(",") && decimal.length > 0 ? `,${decimal}` : "");
