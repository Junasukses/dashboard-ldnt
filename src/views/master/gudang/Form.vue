...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiBallotOutline" title="Cabang" main>
      ...
      <CardBox>
        <FormField>
          <div class="grid grid-cols-3 gap-8">
            ...
            <div>
              <label class="block font-semibold mb-2">Cabang</label>
              <FieldSelect
                :bind="{ disabled: !actionText, clearable: false }"
                class="w-full !mt-0"
                :value="data.m_branch_id"
                @input="(v) => (data.m_branch_id = v)"
                :hints="formErrors.m_branch_id"
                ...
                displayField="name"
                :api="{
                  url: `${store.server}/operation/m_branch`,
                  headers: {
                    'Content-Type': 'Application/json',
                    Authorization: `Bearer ${token}`
                  },
                  params: {
                    where: `this.is_active='true' AND this.m_bu_id=${data.m_bu_id ?? 0}`
                  }
                }"
                label=""
              />
