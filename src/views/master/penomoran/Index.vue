    ...
    <LayoutAuthenticated>
        <SectionMain>
            ...
            <div class="flex gap-4 mb-4">
                <div class="flex items-center p-2 rounded-md cursor-pointer duration-300 transition border border-[#086968]"
                    :class="activeTabIndex == 1
                        ? 'text-white bg-[#086968]' : 'text-[#086968] hover:bg-gray-100 bg-white'
                        " @click="changeActiveTab(1)">
                    <BaseIcon :path="mdiAccountGroupOutline" size="20" />
                    <span class="ml-2">Format Penomoran</span>
                </div>
                <div class="flex items-center p-2 rounded-md cursor-pointer duration-300 transition border border-[#086968]"
                    :class="activeTabIndex == 2
                        ? 'text-white bg-[#086968]' : 'text-[#086968] hover:bg-gray-100 bg-white'
            ...
            <CardBox class="border border-[#086968]">
                <FormField v-show="activeTabIndex === 1">
                    <TableApi ref="formatApiTable" :api="landing_format.api" :columns="landing_format.columns"
                        :actions="landing_format.actions" class="max-h-[450px]">
                        <template #header>
                            <BaseButton @click="router.replace(`${this.$route.path}/format/create`)" type="submit"
                                color="info" label="Create New" outline small />
                        </template>
                    </TableApi>
                </FormField>
                <FormField v-show="activeTabIndex === 2">
                    <TableApi ref="landingApiTable" :api="landing_prefix.api" :columns="landing_prefix.columns"
                        :actions="landing_prefix.actions" class="max-h-[450px]">
                        <template #header>
                            <BaseButton @click="router.replace(`${this.$route.path}/prefix/create`)" type="submit"
                                color="info" label="Create New" outline small />
                        </template>
                    </TableApi>
                </FormField>
